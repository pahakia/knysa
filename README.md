# knysa
knysa is a javascript library that enables you to write phantomjs scripts in "async-await" style, 
i.e. without callbacks.
...
Asynchronous-await(async-await) function must be prefixed with 'knysa_' which informs knysa to suspend and
resume execution.  Such function must resume the execution as follows:
...
2. inside browser (i.e. in knysa_evaluate function), do:
   ...
   The 'data' passed into callPhantom including kflowId will be assigned to ret in the following:
      var ret = kflow.knysa_evaluate(myFunc, kflow.getId()[, additional args]); 
```
...
3. async-await function calls are supported inside:
      a. try/catch/finally blocks
      b. if/else/while blocks
   async-await function calls are NOT supported inside 'for' block or 'switch/case' block.
4. async-await function calls must be on separate line, either
      knysa_my_func(...); or
      ret = knysa_my_func(...)
